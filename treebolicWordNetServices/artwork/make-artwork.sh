...
splash_res=([mdpi]=144 [hdpi]=216 [xhdpi]=288 [xxhdpi]=432 [xxxhdpi]=576)
...
declare -A icon2_res
icon2_res=([mdpi]=32 [hdpi]=48 [xhdpi]=64 [xxhdpi]=96 [xxxhdpi]=128)
icon2_list="ic_action_done.svg ic_action_error.svg"
...
for svg in ${icon2_list}; do
	for r in ${!icon2_res[@]}; do 
		d="${dirres}/drawable-${r}"
		mkdir -p ${d}
		png="${svg%.svg}.png"
		echo "${svg}.svg -> ${d}/${png} @ resolution ${icon2_res[$r]}"
		inkscape ${svg} --export-png=${d}/${png} -h${icon2_res[$r]} > /dev/null 2> /dev/null
	done
done
